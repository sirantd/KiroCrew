...
- **A namespace-scoped export ships only the scoped documents' state rows** — an out-of-namespace row always fails the group-inside-inserted-items test on import, so shipping it would carry another namespace's `file_path`, document name and content hashes out of the machine for nothing. Filtered in `export_all`'s namespace arm, exactly as `source_locations` already is; an unscoped export still ships every row.
