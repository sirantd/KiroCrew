...
That carry-forward adds a disk READ to a function that already wrote, so it sharpens an obligation the write already carried: `write_tombstone` is synchronous and must not run on the gateway's event loop. The three callers that supply no `outcome` — `mark_delivered` on the delivery path and the two `gateway_restart` writes in orphan reconciliation and orphan notification — are all coroutines, and each hands the call to a thread, mirroring the drained path's existing `await asyncio.to_thread(mark_delivered, agent_id)`. Note the namespace: a manager method whose name ends in `_impl` executes with the FACADE's globals, so inside one the module's own private alias is not defined and the offload must name `asyncio`. The remaining `write_tombstone` callers pass an explicit `outcome`, take no read, and keep their existing behaviour.
