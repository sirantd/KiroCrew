...
- **This channel holds it** (the user asked here and follows up here, which is the
  ...
  **dropped with a notice** -- answering it into a session the user left, or running
  ...
  the dashboard drain gives a queued prompt whose admission lapsed. The lookup runs
  twice: at admission, and again in the driver's closing gate -- the yield-free step
  right before the prompt opens (`session_resume.check_replay_binding`, raising
  `ReplayBindingChanged`; the shared pipeline takes it as
  `ChannelTurn.binding_still_holds`) -- because every await between the two
  (callback admission, session acquisition, attachment I/O, the context build) is a
  window in which an unlink or a rebind can land, and a replay that passed the first
  lookup would otherwise run and persist in the session the conversation has left.
  A gate-time mismatch drops the entry with the same notice: nothing runs, nothing
  is persisted, the lease is released.
...
- **The drop reaches the person who sent it.** The dashboard's own drop notice lands
  ...
  without it every DM notice would be refused. A Slack
  ...
  client at the stamped thread instead. Best-effort,
