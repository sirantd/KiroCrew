...
it and compared in the same hook, so a read whose checkout has since been re-resolved is refused
before it spawns -- the clearance proved against the old checkout says nothing about the one now
configured. Both refusals are transient like every other one here: the next poll reads the
