...
refuse. ON on an already-active row re-records its owner entry under the same
slot lock before answering success, repairing an outside resume or invalidated
record without rewriting the live loop. ON on a STOPPED legacy loop is an
