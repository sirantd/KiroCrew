...
NON_EGRESS_REDACTION_MODULES: frozenset[str] = frozenset(
    {
        ...
        # command fragment for a `BANNED` line out of shapes that can hold no
        # secret -- a program basename, an option name with its value dropped, the
        # cap flag's digits -- and withholds every other token, counting it as
        # `+<n>`. What matches the call-site scan here is the name of that builder
        # (`_redacted_command`), not a call to a redactor: nothing is scrubbed,
        # because nothing unrecognised is emitted in the first place. Registering
        # it as a sink would make the panel claim this path is covered BY
        # redaction, when what covers it is withholding -- the same overstatement
        # the decision-seam entry above refuses for refusal.
