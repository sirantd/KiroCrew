...
- **Put scratch work in your OWN subdirectory of `$KIROCREW_SCRATCH` -- not `/tmp`, and not its top level.** Pick one distinctive name for your session (a task slug, never `out`, `tmp` or `work`), `mkdir -p` it once, and keep using it: no environment variable names your session, so the name has to be one you choose and reuse. Clones, probe scripts, build logs, screenshots, and pytest `--basetemp` belong there: the tree is reclaimed automatically once the session's processes are gone, while files in the shared `/tmp` outlive their session, pile up for weeks, and get deleted by age -- including under work that is still live. The subdirectory is a requirement rather than hygiene: `$KIROCREW_SCRATCH` belongs to the PROCESS, and with `agent.chat_runtime_sharing` on an unrelated dashboard chat can share your process and therefore your `$KIROCREW_SCRATCH` and `$TMPDIR` -- so a plainly named file at the top level (`report.md`, `out.tsv`) is one that session can read or overwrite. Scratch is never visible between processes either way. Your sub-agents see the SAME `$KIROCREW_SCRATCH` you do, so a brief or manifest you stage in your subdirectory is readable by every one of them -- hand them its full path, which they cannot derive from a name only you know. `$TMPDIR` is the process's own temp dir (`mktemp`/`tempfile` land there): a sub-agent in its own process does not see it while one sharing your process does, so anything another process must read goes under `$KIROCREW_SCRATCH` by name. State that must outlive your session's processes belongs in NEITHER: `/tmp` is deleted by age under live work, and `$KIROCREW_SCRATCH` is reclaimed once those processes are gone. A directory a LATER run has to find again is not yours to pick: say so and ask, rather than choosing a path under `/tmp` because it visibly survives.
