...
# ``(path, mtime_ns)`` pairs already warned about. The client re-asks for its
# policy on every ``tools/call`` and a refusal is never memoized, so without
# this the gateway log would carry one WARNING per refused tool call for as
# long as the file stays broken. A fix or a re-break changes the mtime and is
# logged again. Bounded so a directory churning broken files cannot grow it
# without limit; clearing it costs one repeated line, nothing else.
_UNREADABLE_SPEC_WARNED: set[tuple[str, int]] = set()
...
def _read_managed_tool_policy_uncached(agents_dir: Path, agent_name: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        # propagate the glob's ``OSError``): no file to name, so the directory
        # is what the reason points at.
        if spec_path is None:
            raise ManagedToolPolicyUnreadable(
                f"agent spec for {agent_name!r} could not be read: the agents directory "
                f"could not be scanned ({exc.__class__.__name__})"
            ) from exc
