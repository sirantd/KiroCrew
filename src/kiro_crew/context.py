...
def _reply_style_rules(level: str) -> str:
    ...
    if level == "answer_only":
        return (
            ...
            "repeats them. A grid of short labels and numbers is a plain "
            "markdown table. When your instructions carry an Inline Widgets "
            "section and the picture needs color, size, position or motion, "
            "it IS an inline widget (an HTML artifact when it is large). "
            "Never a table of sentences. On any other "
    ...
    def _resolve_prompt_templates(prompt: str, session_key: str, cap_figure: str = "") -> str:
        ...
        if density == "more":
            widget_block = (
                ...
                "unreadable in dark mode. Animate only when the motion itself "
                "carries information (change over time, ordered steps, flow, "
                "before/after); if one frozen frame loses nothing, keep it still. "
                "An animation gets a pause control, respects the reduced-motion "
                "setting, and stops when done unless purely decorative.\n\n"
                "## Artifacts\n\n"
            ...
            widget_block = (
                ...
                "motion carries information; add a pause control and respect "
