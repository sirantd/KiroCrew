...
def test_every_transcript_is_committed_before_the_authority_files(tmp_path):
    ...
    record = keys.authority_record_key(settings)
    last_data = max(i for i, key in enumerate(store.puts) if key not in authority | {record})
    first_authority = min(i for i, key in enumerate(store.puts) if key in authority)
    assert last_data < first_authority


def test_the_completeness_record_is_the_last_object_of_the_cycle(tmp_path):
    """The whole safety of the record rests on this order, so it is pinned on the sequence.

    A record written before the pair would name files a cycle interrupted mid-phase never
    uploaded, and every replacement task would then refuse to boot on a bucket that only
    ever held part of one publication. Written last, an interruption leaves no record at
    ...
    record = keys.authority_record_key(settings)
    assert store.puts[-1] == record
    assert all(i < store.puts.index(record) for i in range(len(store.puts) - 1))
...
def test_a_cycle_that_publishes_one_authority_file_writes_no_record(tmp_path):
    """The record describes a WHOLE pair or it is not written at all.

    A crew whose backend has written one of the two publishes that one, with no failure
    anywhere. A record listing just it would assert that a complete publication contained
    one file, and every later restore would boot from it and let the backend flush its own
    empty view of the other -- reintroducing, through the record, the loss the record is
    here to prevent.
    """
    settings = _settings(tmp_path)
    ...
    assert keys.authority_record_key(settings) not in store.puts
    assert keys.AUTHORITY_NAMES[0] in result.gone


# --- 11. two absences, told apart by the completeness record -----------------------
...
class _UnreadableRecord(_Bucket):
    """A bucket whose record cannot be read, which is not the same as not holding one."""

    def get(self, key: str, *, limit: int) -> bytes:
        if key.endswith(keys.AUTHORITY_RECORD_NAME):
            raise PermissionError("the record is there and this task may not read it")
        return super().get(key, limit=limit)


def _record(*names: str) -> bytes:
    return json.dumps({"authority": sorted(names)}).encode("utf-8")


@pytest.mark.parametrize("present", list(keys.AUTHORITY_NAMES))
def test_half_a_pair_with_no_record_is_read_as_never_published(tmp_path, present):
    """The case that must BOOT: one file, and no record saying a pair was ever whole.
...
def test_a_record_naming_a_file_the_bucket_lost_refuses_the_boot(tmp_path, missing):
    ...
    """
    settings = _settings(tmp_path)
    present = {name: b"{}" for name in keys.AUTHORITY_NAMES if name != missing}
    present[keys.AUTHORITY_RECORD_NAME] = _record(*keys.AUTHORITY_NAMES)

    with pytest.raises(restore_mod.RestoreFailed, match="does not hold"):
        restore_mod.restore_authority(settings, _Bucket(present))


def test_a_record_whose_named_files_are_all_there_restores_them(tmp_path):
    """The ordinary case: a whole pair with a record naming it."""
    settings = _settings(tmp_path)
...
def test_a_record_that_cannot_be_read_refuses_the_boot(tmp_path):
    """Unreadable is not absent, and only absent is permission to boot on a partial pair.

    Reading a denial as "no record" would hand back the very boot the record gates, so the
    ...
    with pytest.raises(restore_mod.RestoreFailed, match="could not be read"):
        restore_mod.restore_authority(settings, _UnreadableRecord({keys.AUTHORITY_NAMES[0]: b"{}"}))


@pytest.mark.parametrize("raw", [b"not json", b"[]", b'{"authority": "session_map.json"}'])
def test_a_record_that_does_not_parse_refuses_the_boot(tmp_path, raw):
    """A record present but unusable cannot say which files were published together."""
    settings = _settings(tmp_path)
    present = {keys.AUTHORITY_NAMES[0]: b"{}", keys.AUTHORITY_RECORD_NAME: raw}
