...
def restore_allowed_links(
    ...
    A message saved before the reader allowed a host still carries that host's
    placeholders, and its records keep the full address. For each placeholder
    ...
    was written with. Placeholders pair with records per host in order, the
    same pairing the page uses. Returns the text and the records left.
    """
    if not hosts or not records or EXFILTRATION_REDACTION_TAG_PREFIX not in text:
        return text, records
    queues: dict[str, list[int]] = {}
    for i, rec in enumerate(records):
        queues.setdefault(str(rec.get("domain", "")).lower(), []).append(i)
    ...
    def swap(m: re.Match[str]) -> str:
        ...
        if (
            domain in hosts
            and rec.get("rule") in ALLOWABLE_BLOCKED_LINK_RULES
