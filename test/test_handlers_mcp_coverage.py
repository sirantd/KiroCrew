...
class TestProbe:
    ...
    def test_pinned_purge_removes_exactly_the_pinned_entries(
        self, sandbox: SimpleNamespace, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """With pins the purge resolves nothing: a scope pinned to a key removes
        that key even when the name has since become ambiguous there, a scope
        pinned to ``None`` is a ``noop`` even when an entry appeared since, and
        a pinned key that is already gone is a ``noop``."""
        import kiro_crew.dashboard.handlers.mcp as handler
        ...
        agents_dir.mkdir()
        monkeypatch.setattr(handler, "kiro_agents_dir", lambda: agents_dir)
        pins, ambiguous = mcp_mod._pin_uninstall_keys(["team-foo", "absent"])
        assert ambiguous == {}
        assert pins["team-foo"] == {"kirocrew": "npm:@team/foo", "kiroGlobal": "npm:@team/foo", "agent": None}
        assert pins["absent"] == {"kirocrew": None, "kiroGlobal": None, "agent": None}
        # Edits after the pin: the shared file gains a colliding server, the store
        # loses its entry, and a server named ``absent`` appears in the store.
        _write_global(
            sandbox, {"npm:@team/foo": {"command": "a"}, "other:team/foo": {"command": "b"}}
        )
        store.write_text(json.dumps({"mcpServers": {"absent": {"command": "new"}}}))
        assert mcp_mod._purge_server_config("team-foo", pinned=pins["team-foo"]) == {
            "kirocrew": "noop",
            "kiroGlobal": "removed",
        }
        assert _read_global(sandbox) == {"other:team/foo": {"command": "b"}}
        assert mcp_mod._purge_server_config("absent", pinned=pins["absent"]) == {
            "kirocrew": "noop",
            "kiroGlobal": "noop",
        }
        assert json.loads(store.read_text())["mcpServers"] == {"absent": {"command": "new"}}
