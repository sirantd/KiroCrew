...
/**
 * The text of an inline code span, as the reader sees it — what its chip
 * copies, names itself after, and probes as a path.
 *
 ...
 * hold two lines — `printf aprintf b` is a different command. Every other
 * element contributes its text.
 */
function codeTextOf(node: React.ReactNode): string {
  ...
  if (isElementWithProps(node)) {
    ...
    if (node.props.children != null) return codeTextOf(node.props.children)
  }
  return ''
}
 ...
 *
 * Its click copies, so it wears plain CODE styling: `CHIP_BASE` with no accent
 * colour and no underline, unlike the `CHIP_ACTIONABLE` look of the chips whose
 * click navigates. Dressed as a link, this chip gets clicked for navigation and
 * answers with a silent clipboard write. The copy cursor and the tooltip are
 * what name the action instead.
 *
 ...
 *    the failure once, from the notice's own `role="alert"` — the status region
 *    stays empty then, so nothing is announced twice.
 *
...
function CopyableCode({ className, safeProps, text, children }: {
  ...
  const cue = failed
    ? <CopyFailedNotice placement="bubble" />
    : copied
...
function InlineCode({ children, ...props }: { children?: React.ReactNode } & Record<string, unknown>) {
  ...
  // REPLACES the visible content a screen reader could read before.
  const codeStr = codeTextOf(children).replace(/\n$/, '')
  const probeEnabled = useContext(PathProbeCtx)
  const actions = useContext(PathActionCtx)
  const sessionActions = useContext(SessionActionCtx)
  ...
  if (pathResolution.probePending
    ...
    // Nothing to copy: a raw-HTML span whose only content is an image (`img` is
    // allowlisted) has no visible text, and a button there would write '' —
    // clearing the clipboard — and then confirm it. Stay an inert code span.
    if (raw === '') return <code className={CHIP_BASE} {...safeProps}>{reserve}{children}</code>
