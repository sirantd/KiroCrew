...
function TerminalAction({ title, note, command, label, primary = true }: {
  title: React.ReactNode
  note?: string
  command: string
  label?: string
  ...
  return (
    <>
      <ActionRow title={title} note={note} command={command}>
        <button
          type="button"
      ...
      <Reveal show={state !== 'idle'} contentKey={shownState ?? undefined}>
        ...
        {shownState === 'failed' && <ErrorLine>{i18nT('components.redaction.term_failed')}</ErrorLine>}
      </Reveal>
    </>
  )
}
  ...
  return (
    <CardShell id={id} title={i18nT(session ? 'components.redaction.cred_title_session' : 'components.redaction.cred_title')} testId="credential-card">
      ...
      <Disclosure summary={i18nT('components.redaction.more_ways')} testId="credential-more">        {session && record.view_command && (
          <TerminalAction title={i18nT('components.redaction.cred_view_token_title')} note={i18nT('components.redaction.cred_view_note')} command={record.view_command} primary={false} />
        )}
        {src?.type === 'file' && (
  ...
  return (
    <div data-testid="blocked-link-entry">
      ...
      <Reveal show={panel !== null} contentKey={shownPanel ?? undefined}>
        ...
        {shownPanel === 'error' && shownError && <ErrorLine testId="blocked-link-error">{shownError}</ErrorLine>}
