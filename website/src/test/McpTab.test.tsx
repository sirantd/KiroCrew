...
const mockCopy = vi.hoisted(() => vi.fn().mockResolvedValue(true))
...
describe('McpTab disabled-in-config rows', () => {
  ...
  it('reports a refused copy in place through ErrorNotice, never as Copied', async () => {
    // `copyToClipboard` resolves false when no clipboard API exists (a plain-HTTP
    // gateway) or the browser refuses. That outcome is an error the user must
    // see: it renders through `ErrorNotice` (the repo's one error surface) and
    // the chip stays out of its "Copied" state. Red under `if (!ok) return`:
    // nothing rendered, so the user could not tell the path was not copied.
    ...
    expect(chip.getAttribute('title')).toBe('Copy path')
    // Dismissable, and a later successful copy clears it on its own.
