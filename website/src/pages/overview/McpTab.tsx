 ...
 * the catalog string wraps in `<file>...</file>`.
 */
function CopyPathChip({ path, children }: { path: string; children?: ReactNode }) {
  const [copied, setCopied] = useState(false)
  const [copyFailed, setCopyFailed] = useState(false)
  const timer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined)
  useEffect(() => () => clearTimeout(timer.current), [])
  // The "Copied" state is gated on the clipboard ACTUALLY taking the text (see
  // `copyToClipboard`); a refusal -- no clipboard API on a plain-HTTP gateway,
  // or permission denied -- is reported in place through `ErrorNotice`, the
  // repo's one error surface, never flashed as success or dropped.
  const copy = () => {
        ...
        <table ref={attachMcpTable} className="w-full border-collapse table-striped"><thead><tr>
          ...
          <tbody>{servers.length === 0 ? <tr><td colSpan={6} className="text-muted italic px-2.5 py-3.5 text-sm">{i18nT('pages.overview.mcpTab.no_mcp_servers_configured')}</td></tr> : sortedServers.length === 0 ? <tr><td colSpan={6} className="text-muted italic px-2.5 py-3.5 text-sm">{i18nT('pages.overview.mcpTab.no_matching_servers')}</td></tr> : sortedServers.map((s, i) => {
            ...
            const disabledInConfigWhere = s.disabledInFile
              ? <Trans i18nKey={whereKey} values={{ file: s.disabledInFile }} components={{ file: <CopyPathChip path={s.disabledInFile} /> }} />
              : disabledInConfigWhereText
