...
test("client-only: the local-start offer routes through a re-exec on a crew's port", () => {
  ...
  // Demoted, Retry is the one control whose target the surrounding sentence does
  // not give: that paragraph is about a local start, so a bare label reads as
  // retrying it. Only when a crew is actually named -- there is nothing else to
  // put in the label.
  assert.match(source, /retryLabel: `Retry \$\{remoteTarget\}`/);
  assert.match(source, /remoteTarget \? \{ retryLabel/);
